...
//     //   console.log((document.querySelector(".Trendingcards").getElementsByTagName("div")));
...
//                                 <div class="title">${responce.title}</div>
//                                 <div class="author">${responce.description}</div>
//                                 <div class="playbtn"><img src="./Asset/play.svg" alt=""></div>
//                             </div>`
//         }
//     }
    ...
    for (const song of songs) {
        songUL.innerHTML += `
            <div class="Trendingcard">
                ...
                <div class="title">${song.replaceAll("%20", " ")}</div>
                <div class="author">${data.description || "Unknown"}</div>
                <div class="playbtn"><img src="./Asset/play.svg" alt=""></div>
            </div>`;
    }

    // Attach event listener to play song
    Array.from(document.querySelector(".Trendingcards").getElementsByClassName("Trendingcard")).forEach(e => {
...
async function displayAlbums() {
    ...
    for (const folder of albums) {
        let res = await fetch(`./Asset/songs/${folder}/playlist.json`);
        if (!res.ok) continue;
        let data = await res.json();

        authorcards.innerHTML += `
            <div data-folder="songs/${folder}" class="authorcard">
    ...
    document.querySelector(".range input").addEventListener("change", (e) => {
        ...
        if (currentSong.volume > 0) {
            document.querySelector(".volume>img").src =
                document.querySelector(".volume>img").src.replace("mute.svg", "audio.svg");
        }
    });
}
